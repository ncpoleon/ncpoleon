...
def test_max_cut_ncpol2sdpa(benchmark, n_vars, level):
    from ncpol2sdpa import SdpRelaxation, generate_variables

    variables = generate_variables("x", n_vars)
    ...
    sdp = SdpRelaxation(variables)
    benchmark(sdp.get_relaxation, level, objective=-objective, substitutions=substitutions)
